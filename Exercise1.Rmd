...
ggplot(above_summ, aes(x=green_rating, y=above.mean)) + 
  geom_bar(stat='identity')
```

```{r, greenbuildings2, echo=FALSE}
greenbuildings$rentAboveAverage = greenbuildings$Rent - greenbuildings$cluster_rent
...
```{r, greenbuildings3a, echo=FALSE}
# gas cost
gb = subset(greenbuildings, green_rating == 0)
gb$green_rating[gb$green_rating == 0] <- 'NOT GREEN'
ggplot(data = gb) + 
  geom_point(mapping = aes(x = hd_total07, y = Gas_Costs, color = green_rating))
```

```{r, greenbuildings3b, echo=FALSE}
# gas cost
gb = subset(greenbuildings, green_rating == 1)
gb$green_rating[gb$green_rating == 1] <- 'GREEN'

ggplot(data = gb) + 
  geom_point(mapping = aes(x = hd_total07, y = Gas_Costs, color = green_rating))
```

```{r, greenbuildings3, echo=FALSE}
# gas cost
gb = greenbuildings
gb$green_rating[gb$green_rating == 0] <- 'NOT GREEN'
gb$green_rating[gb$green_rating == 1] <- 'GREEN'
...
```{r, greenbuildings4a, echo=FALSE}
# gas cost
gb = subset(greenbuildings, green_rating == 0)
ggplot(data = gb) + 
  geom_point(mapping = aes(x = cd_total_07, y = Electricity_Costs, color = green_rating))
```



```{r, greenbuildings4b, echo=FALSE}
# gas cost
gb = subset(greenbuildings, green_rating == 1)
ggplot(data = gb) + 
  geom_point(mapping = aes(x = cd_total_07, y = Electricity_Costs, color = green_rating))
...
```{r, greenbuildings5a, echo=FALSE}
# age vs rent
gb = subset(greenbuildings, green_rating == 1)
ggplot(data = gb) + 
  geom_point(mapping = aes(x = age, y = Rent, color = LEED))
```

```{r, greenbuildings5b, echo=FALSE}
gb = subset(greenbuildings, green_rating == 1)
gb$rentAboveAverage = gb$Rent - gb$cluster_rent

above_summ = gb %>%
  group_by(LEED)  %>%  # group the data points by model name
  summarize(above.mean = mean(rentAboveAverage))  # calculate a mean for each model

ggplot(above_summ, aes(x=LEED, y=above.mean)) + 
  geom_bar(stat='identity')
```

```{r, greenbuildings5c, echo=FALSE}
gb = greenbuildings
...
ggplot(data=greenbuildings) + 
  geom_boxplot(mapping=aes(x=green_rating, y=age))
```

```{r, greenbuildings6, echo=FALSE}
