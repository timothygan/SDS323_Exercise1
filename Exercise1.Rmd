...
greenbuildings$rentAboveAverage = greenbuildingsrent - greenbuildings$cluster.rent
above_summ = greenbuildings %>%
  group_by(green.rating)  %>%  # group the data points by model name
  summarize(above.mean = mean(rentAboveAverage))  # calculate a mean for each model

ggplot(security_summ, aes(x=green.rating, y=above.mean)) + 
